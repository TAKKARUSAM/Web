...
// NOTE: the uploaded file `pularaadha.mp3` is checked at runtime. Do NOT import it statically
// because the file may not exist yet and that would break the dev server build.
...
export default function ChillZone({ onNext }: ChillZoneProps) {
  // Tracks are stored in state so we can detect the presence of the uploaded MP3 at runtime
  const [tracks, setTracks] = useState<Track[]>([
    { id: 1, title: textConfig.chillZone.tracks[0].title, caption: textConfig.chillZone.tracks[0].caption, src: music3, cover: cover2 },
  ]);

  // On mount, check if the uploaded file exists at /src/music/pularaadha.mp3.
  // If present, use it as track 1 (keeping the same cover). If not, fall back to music3 only.
  useEffect(() => {
    let mounted = true;
    const checkUploaded = async () => {
      // Try both common locations: src/music and public root
      // Check lowercase and capitalized filenames to handle case differences on different OS/servers
      const candidates = [
        '/src/music/pularaadha.mp3',
        '/src/music/Pularaadha.mp3',
        '/pularaadha.mp3',
        '/Pularaadha.mp3',
      ];
      try {
        for (const path of candidates) {
          try {
            ...
            if (res.ok) {
              if (!mounted) return;
              setTracks([
                { id: 1, title: textConfig.chillZone.tracks[0].title, caption: textConfig.chillZone.tracks[0].caption, src: path, cover: cover2 },
                { id: 2, title: textConfig.chillZone.tracks[1].title, caption: textConfig.chillZone.tracks[1].caption, src: music3, cover: cover3 },
              ]);
              return;
            }
          } catch (e) {
        ...
        if (!mounted) return;
        setTracks([
          { id: 1, title: textConfig.chillZone.tracks[1].title, caption: textConfig.chillZone.tracks[1].caption, src: music3, cover: cover3 },
        ]);
      } catch (err) {
        if (!mounted) return;
        setTracks([
  ...
  // Scroll functions (renamed to avoid shadowing/destructuring collisions)
  const scrollPrev = () => {
    if (!scrollContainerRef.current) return;
  ...
  useEffect(() => {
    // Run a layout-safe check after the component mounts so measurements are correct
    let raf = 0 as number;
    const runCheck = () => {
      checkScrollPosition();
    };

    raf = requestAnimationFrame(runCheck);
